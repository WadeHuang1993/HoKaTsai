        ...
        <div class="card-body">
            <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value="{{ old('start_date', $course->start_date->format('Y-m-d\TH:i')) }}" required>
                    @error('start_date')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" value="{{ old('end_date', $course->end_date->format('Y-m-d\TH:i')) }}" required>
                    @error('end_date')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('location') is-invalid @enderror" id="location" name="location" value="{{ old('location', $course->location) }}" required>
                    @error('location')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($course->image)
                        <div class="mb-2">
                            <img src="{{ Storage::url($course->image) }}" alt="課程封面圖" class="img-thumbnail" style="max-height: 200px;">
                        </div>
                    @endif
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('team_member_id') is-invalid @enderror" id="team_member_id" name="team_member_id" required>
                        ...
                        @foreach($teamMembers as $teamMember)
                            <option value="{{ $teamMember->_id }}" {{ old('team_member_id', $course->team_member_id) == $teamMember->_id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div id="schedule-container">
                        @foreach($course->schedule as $index => $schedule)
                            <div class="schedule-item mb-3">
                                <div class="row">
                                    <div class="col-md-5">
                                        <input type="text" class="form-control" name="schedule[{{ $index }}][time]" value="{{ $schedule['time'] }}" placeholder="時間" required>
                                    </div>
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" name="schedule[{{ $index }}][content]" value="{{ $schedule['content'] }}" placeholder="內容" required>
                                    </div>
                                    <div class="col-md-1">
                                        <button type="button" class="btn btn-danger remove-schedule" {{ count($course->schedule) <= 1 ? 'style=display:none' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control @error('max_participants') is-invalid @enderror" id="max_participants" name="max_participants" value="{{ old('max_participants', $course->max_participants) }}" min="1" required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control @error('remaining_slots') is-invalid @enderror" id="remaining_slots" name="remaining_slots" value="{{ old('remaining_slots', $course->remaining_slots) }}" min="0" required>
                ...
                <div class="form-group">
                    ...
                    <input type="url" class="form-control @error('google_form_url') is-invalid @enderror" id="google_form_url" name="google_form_url" value="{{ old('google_form_url', $course->google_form_url ?? '') }}">
                ...
                <div class="form-group">
                    ...
                    <div id="notes-container">
                        @foreach($course->notes as $index => $note)
                            <div class="note-item mb-3">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="notes[]" value="{{ $note }}" placeholder="注意事項" required>
                                    <div class="input-group-append">
                                        <button type="button" class="btn btn-danger remove-note" {{ count($course->notes) <= 1 ? 'style=display:none' : '' }}>
                                            <i class="fas fa-times"></i>
                                        </button>
                                    </div>
                                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // 新增課程流程
            let scheduleCount = {{ count($course->schedule) }};
